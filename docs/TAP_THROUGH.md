...
- [ ] **Passenger 1's fare goes back up to what they originally booked** (30, not 22.50) and their
      ...
      the job they accepted. It must never go _above_ what they booked at
...
**NOTE**
Now, the truth of the matter is there are certain things I won't be able to test, especially since we don't have a dev build. Which, by the way, we have to do for all three apps, and we also have to host the back end because now it's a requirement. And just so you know, our deadline is in a few hours. So we are really behind. So things like the map, the card services, and others, I won't be able to test them very well. Another reason is that I have a limited number of devices, but I'm trying to run three apps at the same time. So I'm relying heavily on you to make sure that everything is done right. That is why there are a lot of blank spaces in the tap-through document. Some I just can't test; others are not a direct true-or-false situation. So that's why I just list the issues I find. So that you can fix all those issues and bugs.
It looks like the persistence I was talking about for the recent searches still isn't working because when I logged in again, the recent searches were cleared and there were no past searches. Also, how is the login like? Because at first, when I open the app in Expo Go, I'm logged in right into the last account that was logged in, which is good. But right now, anytime I rebuild the app, I have to log in. Sometimes it t looks like it has logged me back in, but nothing is working. So, I have to log out and log back in for the services to work.
We have a major issue with the payment system where the user is able to click done and leave the payment screen or page without making payment. And as if that is not bad enough, the user can't go back in case it was a mistake to finish the payment. The driver is never credited with the money. So it shows on the driver app that the driver is still waiting for the payment, but the payment never comes. And this might be an issue mainly with cash payments. What actually happened was that, right at the payment for the ride, I changed the payment method from the wallet to cash. Then I clicked Done to test.
Check how ratings are calculated and, if there is any issue, fix it because I have made two trips with a particular driver and I rated them four out of five on just one. But their rating is 4.9. That doesn't make sense. Also, when the driver is new and has never had a trip or has never been rated, the rating should just be zero. There shouldn't be that new text over there.
For the parcel, it's working all right. But after the customer is done or has paid for the parcel or delivery, it shouldn't see " Send another parcel. It should just see done. Also, the customer or user's name does not appear on the driver app. So for both delivery of food, products, or parcels, the name never appears. So with that, drivers or couriers can't verify the identity of a customer. For parcels, the name of the person it's going to be delivered to shows, which is good, but it should also show the sender's details, just in case of anything. Also, after one of the parcel deliveries, I'm not very sure what happened, but then the driver mistakenly exited the trip without confirming that the cash had been received. So, the customers are saying that it is waiting for the driver to confirm that they've received the cash, but the driver can't do that on the app because the trip no longer exists. So, fix that, and the drivers and couriers should also have trip history. Also, debts should be deducted directly from the driver's wallet if there is enough money in it. If the money in it isn't enough. it will show the "You owe GoZone" banner and will be paid with either Momo or card.
For the vendor app, there should be a new " Your Details " section under Account on the profile screen because we've moved it into the banner or button at the top. Also, the menu items on the menu or catalogue should be editable. So, you can add images to the ones that didn't have them during setup. And you can change the contents of the items
Also, just by the way, during testing I kept getting orders that have not been made. They come on and go off. Even to get it, it was one particular order: it comes on and goes off, and in the driver or courier app it also kept showing a food delivery for that particular order that kept showing up on the vendor app. This was happening while you were working on the other features. So I don't know if it is as a result of you working in the directory. But you can just confirm to make sure that everything is good.
Also, just by the way, during testing I kept getting orders that have not been made. They come on and go off. Even to get it, it was one particular order, and it comes on and goes off, and in the driver or courier app it also kept showing a food delivery for I think that particular order that kept showing up on the vendor app. Now this was happening whilst you were working on the other features. So I don't know if it is as a result of you working in the directory. But you can just confirm to make sure that everything is good.
The system should give more information when a user clicks on SOS or reports an incident. For instance, the name of the driver, the name of the user themselves, and other necessary information. I know the location is added, which is good. And we can even make it a live location. So, it can track both the driver assigned to that customer and the customer themselves.
Also set up the Google sign-in and the forgotten password
Clear the databases so the entire system and app feel new; for example, the old delivery requests in the driver app. For the seeded data, make sure they are intact and realistic; for example, the names of users, drivers, couriers and vendors should be realistic(it is not required for them to be local; mix them up), and the items and images on the catalogues should be realistic. You could even set up known food chains for accuracy. And before i miss this, for the vendor system, customers should only be able to see shop relatively close to them, or in their region. Also, I am currently in Kumasi, so most of the shops being in Accra isn't helpful. Most importantly, nothing should be mocked; everything has to actually work else we'll be penalised
