...
export function LeafletMap({ style, center, zoom = 14, mode = 'view', markers = [], driver = null, vehicleKind = 'car', userLocation = null, flyTo = null, route = [], onCenterChange, onReady }: Props) {
   ...
   *
   * <p>Every live update here — route, driver, user dot, flyTo — was fired the moment its prop
   * changed, with nothing checking the map existed yet. `injectJavaScript` on an unloaded WebView
   * and `postMessage` to an unloaded iframe are both silently dropped, so those updates were gone
   * for good. The route was the visible casualty: directions come back in a few hundred
   * milliseconds, usually *before* Leaflet has finished loading, so the real road geometry was
   * discarded and the map kept the straight-line fallback baked into its initial HTML — which is
   * exactly "the route doesn't show".
   */
  ...
  // Send a command to the map, or hold it until the map says it is ready.
