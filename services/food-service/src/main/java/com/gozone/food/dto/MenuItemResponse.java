...
public record MenuItemResponse(
    UUID id, String name, String description, BigDecimal price, boolean available,
    List<Group> groups
) {
    ...
    public static MenuItemResponse from(MenuItem m) {
        List<Group> groups = m.getGroups().stream()
            ...
            .toList();
        return new MenuItemResponse(m.getId(), m.getName(), m.getDescription(), m.getPrice(), m.isAvailable(), groups);
    }
}
