...
import { usePayout } from '../store/payoutStore';

// Wipe every user-scoped store (online/active-trip state, the resumable KYC setup
// draft, vehicle details, account profile, saved payout destination) so no data leaks
// from one driver account into the next. Called on logout and before a fresh login.
export async function clearUserData(): Promise<void> {
  ...
  await usePayout.getState().reset();
}
