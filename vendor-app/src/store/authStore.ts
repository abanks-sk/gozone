...
const EMPTY_ME: MeProfile = { status: null, statusNote: null, name: null, username: null, email: null, phone: null };
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  fetchMe: async () => {
    try {
      ...
      // The token carries the status it was minted with. When an admin approves the business the
      // database changes but the token in hand still reads PENDING, and every endpoint gated on
      // STATUS_ACTIVE keeps refusing. Re-mint as soon as this call notices the disagreement — this
      // is the poll VendorGate uses to open the app up, so the two have to agree.
